...
* **Paths** (where the restricted data lives, where outputs go). These differ per
  machine and must never be committed, so they come from environment variables,
  usually written in a local, git-ignored `.env` file.
* **Parameters** (thresholds, bin sizes, ...). These come from the packaged
  `default_config.yaml`, optionally overridden by a user YAML file (FISH_CONFIG).

Every later pipeline step calls `load_settings()` once and reads from the
returned `Settings` object instead of reading environment variables itself.
"""
...
ENV_VIDEO_DIR = "FISH_VIDEO_DIR"
ENV_DB_PATH = "FISH_DB_PATH"
ENV_REFERENCE_PDF = "FISH_REFERENCE_PDF"
ENV_OUTPUT_DIR = "FISH_OUTPUT_DIR"
ENV_WORKERS = "FISH_WORKERS"
ENV_CONFIG = "FISH_CONFIG"

# Maps the short names used in code (settings.require("video_dir")) to their env var.
PATH_VARIABLES = {
    "video_dir": ENV_VIDEO_DIR,
    "db_path": ENV_DB_PATH,
    "reference_pdf": ENV_REFERENCE_PDF,
}
...
def load_settings(
    ...
    so that Colab / CI can set values without editing files:
    ...
    (or the FISH_CONFIG file) when given.
    """
    ...
    paths = Paths(
        ...
        reference_pdf=_as_path(lookup(ENV_REFERENCE_PDF)),
...
def deep_merge(base: Mapping[str, Any], override: Mapping[str, Any]) -> dict[str, Any]:
    ...
    This lets a user YAML change one value (e.g. `labeling.min_bout_s`)
...
def _parse_workers(value: str | None) -> int:
    """Parse FISH_WORKERS; default 1 (sequential), must be a positive integer."""
    if value is None:
        return 1
    try:
        workers = int(value)
